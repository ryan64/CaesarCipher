...
int main() //The user interface, where the user decides on what he wants the program to do. 
{
	...
	while (number < 1) 
	{
		...
		cout << "\n"; 
		cout << "Instructions: The message you want to encrypt, should be in a file called" << endl; 
		cout << "'text.txt'. Please make sure that the .exe file is located in the same" << endl; 
		cout << "destination as the file you want to encrypt. The 'encryptionKey.txt'" << endl;
		cout << "should also be in the same destination." << endl; 
		cout << "\n";  
		...
		if (key == "en")
		{
			...
			{
				system("CLS"); 
				cout << "The file 'text.txt' could not be found." << endl; 
...
void encrypt(string file)
{
	...
	{
		system("CLS"); 
		cout << "The file 'encryptionKey.txt' could not be found." << endl; 
		cout << "Aborting program...." << endl; 
		system("pause"); 
		exit(1); 
	}
